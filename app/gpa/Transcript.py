...
class Transcript:
    ...
    def get_transcript_content(self) -> list:
        ...
        logger.info(f"Extracted {len(transcript)} course(s) from the transcript")
        return transcript
    ...
    def get_all_info_for_term(self, term_num: int) -> list:
        ...
        if num_of_classes > 0:
            logger.info(f"Found {num_of_classes} classes taken during Term {term_num}")
        else:
            logger.warning(f"No classes was found for Term {term_num}")
        return term_info
    ...
    def split_course_by_attr(self, course_list, attr) -> dict:
        ...
        for course in course_list:
            ...
            if attr_value in split_course.keys():
                logger.info(f"Appending course {course.class_code} to the dict of {attr_value} for {attr}")
                split_course[attr_value].append(course)
            else:
                logger.info(f"Adding course {course.class_code} to the dict of {attr}")
                split_course[attr_value] = [course]
        logger.info(f"Found {len(split_course.keys())} different course types. {attr}: {', '.join(split_course.keys())}")
        return split_course
